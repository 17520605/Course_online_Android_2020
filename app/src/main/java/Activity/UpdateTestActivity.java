...
public class UpdateTestActivity extends AppCompatActivity {
    ...
    File imagefile = null;
    String imagename = "1609835401514-554751128-FAQ-Q-and-A-icon-symbol-questions-answer-Graphics-5337172-1.jpg";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_multichoice_update);
        initUIs();
        initEvents();
        ActionToolBar();
        Load();
    ...
    private void initUIs() {
        TBupdateTest = findViewById(R.id.TBupdateTest);
        question = findViewById(R.id.UpdateTest_Question);
        image = findViewById(R.id.UpdateTest_Image);
        ansA = findViewById(R.id.UpdateTest_A);
        ansB = findViewById(R.id.UpdateTest_B);
        ansC = findViewById(R.id.UpdateTest_C);
        ansD = findViewById(R.id.UpdateTest_D);
        radioA = findViewById(R.id.UpdateTest_A_Rbtn);
        radioB = findViewById(R.id.UpdateTest_B_Rbtn);
        radioC = findViewById(R.id.UpdateTest_C_Rbtn);
        radioD = findViewById(R.id.UpdateTest_D_Rbtn);
        createBtn = findViewById(R.id.UpdateTest_CreateBtn);
        chooseimage = findViewById(R.id.UpdateTest_ChooseImageBtn);
    }
    ...
    private void Load(){
        if(getIntent().hasExtra("TESTS") && getIntent().hasExtra("TEST_ID")){ // load from extra data
            try {
                ...
                for(int i = 0; i < tests.size(); i++){
                    if(tests.get(i)._id.compareTo(testId)==0){
                        MutipleChoiceModel test = tests.get(i);
                        question.setText(test.question);

                        Picasso.get().load("http://149.28.24.98:9000/upload/lesson/" + imagename)
                        ...
                        if(test.answer.compareTo("A") == 0){
                            radioA.setChecked(true);
                        }
                        else
                        if(test.answer.compareTo("B") == 0){
                            radioB.setChecked(true);
                        }
                        else
                        if(test.answer.compareTo("C") == 0){
                            radioC.setChecked(true);
                        }
                        else
                        if(test.answer.compareTo("D") == 0){
    ...
    private void ActionToolBar() {
        setSupportActionBar(TBupdateTest);
        getSupportActionBar().setDisplayShowTitleEnabled(false);
        TBupdateTest.setTitleTextColor(-1);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        TBupdateTest.setNavigationOnClickListener(new View.OnClickListener() {
            @Override
                ...
                .enqueue(new Callback<ResponseBody>() {
                    ...
                    public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                        if(response.isSuccessful()){
                            Toasty.success(UpdateTestActivity.this, "Tải ảnh lên thành công !", Toast.LENGTH_SHORT, true).show();
                            String str = null;
                            try {
                            ...
                            Toasty.error(UpdateTestActivity.this, "Tải ảnh lên thất bại !", Toast.LENGTH_SHORT, true).show();
                        }
                    }

                    @Override
                    public void onFailure(Call<ResponseBody> call, Throwable t) {
                        Toasty.error(UpdateTestActivity.this, "Lỗi !", Toast.LENGTH_SHORT, true).show();
                    }
                });
    }
        ...
        if(isValidate()){
            ...
            service.UpdateTest("lesson/add-list-multiple-choice/" + lessonId,
                    ...
                    .enqueue(new Callback<ResponseBody>() {
                        ...
                        public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                            if(response.isSuccessful()){
                                Intent intent=new Intent(UpdateTestActivity.this, ListTestCreatedActivity.class);
                                intent.putExtra("LESSON_ID", lessonId);
                                UpdateTestActivity.this.startActivity(intent);
                                Toasty.success(UpdateTestActivity.this, "Thêm bài tập thành công !", Toast.LENGTH_SHORT, true).show();
                            }
                            else {
                                Toasty.error(UpdateTestActivity.this, "Thêm bài tập thất bại !", Toast.LENGTH_SHORT, true).show();
                            }
